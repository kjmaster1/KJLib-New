...
import com.kjmaster.kjlib.gui.widgets.Button;
import com.kjmaster.kjlib.gui.widgets.Panel;
import com.kjmaster.kjlib.network.PacketOpenManual;
import com.kjmaster.kjlib.network.PacketSetGuiStyle;
...
import static com.kjmaster.kjlib.gui.widgets.Widgets.*;
...
public class GuiSideWindow {
    ...
    public void initGui(final Minecraft mc, Screen gui, int guiLeft, int guiTop, int xSize, int ySize) {
        style = KJLib.getPreferencesProperties(mc.player).map(PreferencesProperties::getStyle).orElse(GuiStyle.STYLE_FLAT_GRADIENT);

        Button helpButton = button(1, 1, 16, 16, "?")
                .tooltips("Open manual")
                .event(() -> help(mc));
        guiButton = button(1, 19, 16, 16, "s")
                .event(() -> changeStyle(KJLib.networkHandler));
        setStyleTooltip();
    ...
    private void help(Minecraft mc) {
        if (manual != null) {
            KJLib.networkHandler.sendToServer(new PacketOpenManual(manual, manualNode, page));
        }
    }
