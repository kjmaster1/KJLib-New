...
import com.kjmaster.kjlib.gui.events.ChannelEvent;
import com.kjmaster.kjlib.gui.events.FocusEvent;
import com.kjmaster.kjlib.gui.widgets.AbstractContainerWidget;
...
//        Minecraft.getInstance().keyboardHandler.setSendRepeatsToGui(true);
    ...
    public <T extends Widget<T>> T findChild(String name) {
        ...
        if (widget == null) {
            KJLib.LOGGER.error("Could not find widget '" + name + "'!");
        }
        return (T)widget;
    }
    ...
    public boolean keyTyped(int keyCode, int scanCode) {
        if (keyCode == GLFW.GLFW_KEY_F12) {
            ...
            try {
                ...
                KJLib.LOGGER.error("Problem writing output.gui!", e);
            }
        }
        if (textFocus != null) {
    ...
    public void draw(GuiGraphics graphics) {
        ...
        currentStyle = KJLib.getPreferencesProperties(gui.getMinecraft().player).map(PreferencesProperties::getStyle).orElse(GuiStyle.STYLE_FLAT_GRADIENT);
    ...
    private void fireFocusEvents(Widget<?> widget) {
        if (focusEvents != null) {
            for (com.kjmaster.kjlib.gui.events.FocusEvent event : focusEvents) {
                 event.focus(widget);
            }
        }
    }
