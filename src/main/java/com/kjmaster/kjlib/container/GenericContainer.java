...
import com.kjmaster.kjlib.api.container.IGenericContainer;
import com.kjmaster.kjlib.network.PacketContainerDataToClient;
    ...
    protected Slot createSlot(SlotFactory slotFactory, Player playerEntity, final IItemHandler inventory, final int index, final int x, final int y, SlotType slotType) {
        ...
        if (slotType == SlotType.SLOT_GHOST) {
            ...
            slot = new CraftingSlot(playerEntity, inventory, te, index, x, y)
                    .onCraft((TriConsumer<BlockEntity, Player, ItemStack>) slotFactory.slotDefinition().getOnCraft());
        } else {
    ...
    public ItemStack quickMoveStack(@Nonnull Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            if (factory.isSpecificItemSlot(index)) {
                ...
                KJLib.LOGGER.info("Weird slot at index: " + index);
            }
    ...
    private void broadcast() {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            for (IContainerDataListener data : containerData.values()) {
                ...
                KJLib.networkHandler.sendTo(packet, serverPlayer.connection.connection, NetworkDirection.PLAY_TO_CLIENT);
            }
        }
    }
